...
description: >
  ...
  of work, Project name, Definition of Done. Use when the user wants a task file
...
Returns the resolved list and, for **Field of work**, **Project name** and **MoSCoW**, the exact option names ClickUp accepts. Suggest only names from this output. The option lists change; the script output is current, this file is not.
...
python3 "$SCRIPT" create --file TASK-x.md --dest sprint \
  ...
  --field-of-work "Boilerplate" --dod "One sentence." --dry-run
```
...
- Three or more open questions in the file: say so before asking for a destination. Triage can absorb them; a sprint task with them is not ready, and the user decides.
